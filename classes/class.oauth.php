...
// echo "SELECT id FROM ".USERS." WHERE mobileNumber = '".$db->filter($_POST['mobileNumber'])."'";
	        ...
	        // }	
	         ...
	         $rs = $db->insert(REGISTERED_USER, $regData);
		        ...
		        $mail->FromName = 'www.desitemplehair.com';
		        $mail->AddAddress($db->filter($_POST['email']));
		        $mail->AddBcc('santhosh@evol.co.in');
		        	             
		        $mail->Subject = 'desitemplehair.com: Registering Confirmation';      
		        $contents = "Dear ".$db->filter($_POST['fullName']).", <br/> Thank you for registering at <a href='http://desitemplehair.com'>desitemplehair.com</a>. This email confirms that you have registred to desitemplehair.com <br/> If you have not registred with desitemplehair.com please contact us.<br>Email: support@desitemplehair.com<br>+91 7618792925";
		        $msg.=$contents.'<br><br>';
	...
	function forgotpassword(){
        ...
        if($rowExits){
        	...
        	if($updated){
		        ...
		        $mail->Subject = 'desitemplehair.com: New Password';      
	...
	function fblogin(){
		...
		$fb = new Facebook\Facebook([
		    'app_id' => '148870929161409',
		    'app_secret' => '04a6cfc4e8f54937c0f3a6d7ada82acf',
		  ...
		  $registerExits = $db->get_row("SELECT id,fullName,email FROM ".REGISTERED_USER." WHERE fbid = '".$user['id']."'",true);
        if($registerExits){
        		$_SESSION['fullName'] = $registerExits->fullName;
				$_SESSION['regId'] = $registerExits->id;
				$_SESSION['email'] = $registerExits->email;
        }else{
	        $data = array(
	            'fullName' => $user['name'],
	            'email' => $user['email'],
	...
	function checkgoogleUserExists($email,$name,$gid){
		$db = new DB();
		date_default_timezone_set("Asia/Calcutta");
        $dateTime  = date('Y-m-d H:i:s', time());
		 $registerExits = $db->get_row("SELECT id,fullName,email FROM ".REGISTERED_USER." WHERE email = '".$email."'",true);
        if($registerExits){
        		$_SESSION['fullName'] = $registerExits->fullName;
				$_SESSION['regId'] = $registerExits->id;
				$_SESSION['email'] = $registerExits->email;
